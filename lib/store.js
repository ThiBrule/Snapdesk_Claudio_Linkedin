...
/** Liste toutes les clés commençant par `prefix` → [{ key, value, updated_at }]. */
export async function kvList(prefix) {
  const { base, key } = cfg();
  const res = await fetch(`${base}?key=like.${encodeURIComponent(prefix + '*')}&select=key,value,updated_at`, { headers: headers(key) });
